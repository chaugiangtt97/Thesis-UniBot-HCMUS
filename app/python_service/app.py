...
OPTIONAL_ENV_VARS = [
    ...
    "MILVUS_PORT",
    "MILVUS_URI",
    "MILVUS_TOKEN" "MILVUS_USERNAME",
    "MILVUS_PASSWORD",
    "APP_DATABASE",
    "MONGO_CONNECTION_STRING",
    "SEARCH_THRESHOLD",
    "LATEST_TIMESPAN_MONTHS",
]
...
def createApp():
    try:
        ...
        from routes.v2.document import main as documentRoute
        ...
        app.register_blueprint(documentRoute, url_prefix="/document")
        ...
        if missing_required_vars:
            raise EnvironmentError(
                f"Missing required environment variables: {', '.join(missing_required_vars)}"
            )
...
if __name__ == "__main__":
    try:
        app = createApp()
        app.run(debug=False, host="0.0.0.0", port=os.getenv("PYTHON_PORT", 5000))
    except Exception as e:
        raise e
