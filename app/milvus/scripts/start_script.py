...
def convert_type_string_to_datatype(type_string):
    ...
    if normalized_type in type_mapping:
        ...
        raise ValueError(
            f"Unsupported data type: '{type_string}'. Available types: {available_types}"
        )
...
def ingest_pipeline(data_path, collection_path):
    ...
    for col in collection_names:
        ...
        schema = CollectionSchema(
            fields, description=collection_data[col]["description"]
        )
    ...
    for col in df["collection"].unique():
        if col in existing_collections:  # Check if collection already exists
            continue
        data = (
            df[df["collection"] == col]
            .loc[:, df.columns != "collection"]
            .dropna(axis=1, how="all")
        )  # Get by type, remove columns with all NaN
        data = data.to_dict("records")
        for d in data:
            d["embedding"] = ast.literal_eval(
                d["embedding"]
            )  # Revert string representation to float array
