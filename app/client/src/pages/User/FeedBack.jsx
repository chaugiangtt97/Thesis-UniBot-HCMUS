...
function FeedBack() {
  ...
  useEffect(() => {
    document.title = t("user_feedback.title")//'Chatbot - Phản Hồi'
  ...
  const submit = (message) => {
    ...
    useAuth.feedback(token, { message }).then(() => {
      noticeHandler.add({
        status: 'success',
        message: t("user_feedback.feedback_received") // 'Phản hồi của bạn được ghi nhận, Xin cảm ơn !'
      })
      setValue(null)
    }).catch((error) => {
      noticeHandler.add({
